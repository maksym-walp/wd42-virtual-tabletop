-- Default starter node per archetype, auto-unlocked at character creation
-- (see CharacterModel.create's root-node INSERT, which matches on
-- is_root=true + the `archetypes` array column). Runs automatically on a
-- fresh database — also present as database/migrations/09-... for
-- already-existing installs that predate this file.

INSERT INTO skill_tree.nodes
  (title, description, cost, narrative_condition, archetype, archetypes, is_root, pos_x, pos_y)
SELECT
  'Початок шляху',
