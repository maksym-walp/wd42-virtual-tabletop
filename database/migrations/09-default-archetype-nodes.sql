...
--
-- narrative_condition was TEXT when this migration was first written
-- and only became TEXT[] later (migration 11). On installs that
-- replay every migration from scratch against an already-current
-- schema (narrative_condition already TEXT[]), a plain string literal
-- fails with "malformed array literal". Branch on the column's actual
-- type at runtime so this works whichever state it's in.
-- ================================================================

DO $$
DECLARE
  narrative_is_array boolean;
BEGIN
  SELECT (data_type = 'ARRAY') INTO narrative_is_array
  FROM information_schema.columns
  WHERE table_schema = 'skill_tree' AND table_name = 'nodes' AND column_name = 'narrative_condition';

  IF narrative_is_array THEN
    ...
    SELECT
      ...
      0,
      ARRAY['Хтось відверто зневажив вас, загрожує вашим союзникам, або мирні аргументи остаточно вичерпали себе. Змусьте супротивника пошкодувати про це, вирішивши питання силою чи безапеляційною демонстрацією власної переваги.'],
    ...
    SELECT
      ...
      0,
      ARRAY['Пізнання світу розпочинається із пізнання себе. Зіткніться з незрозумілим або опиніться в безвиході, де звичайна зброя безсила, та свідомо застосуйте магію уперше, щоб змінити хід подій.'],
    ...
    SELECT
      ...
      0,
      ARRAY['Хочеш жити — умій крутитись. Опиніться у вкрай невигідній або небезпечній ситуації та виплутайтеся з неї, покладаючись виключно на власну вдачу, обман, соціальні маніпуляції або спритність рук.'],
