...
│   └── migrations/             # подальші міграції (застосовувати вручну)
└── services/
...
Для кожного бекенд-сервіса (`auth`, `user-profile`, `spellbook`, `skill-tree`, `character-sheet`, `dice-roller`, `equipment`, `maneuvers`, `abilities`, `campaigns`):
```bash
cd services/<service>
npm install
...
```bash
cd services/<service>   # auth | user-profile | spellbook | skill-tree | character-sheet | dice-roller | equipment | maneuvers | abilities | campaigns
npm test
```

Прогнати тести всіх бекенд-сервісів послідовно:
```bash
for s in auth user-profile spellbook skill-tree character-sheet dice-roller equipment maneuvers abilities campaigns; do
...
- Файли в `database/migrations/*.sql` треба застосовувати вручну на існуючій БД (автоматичного застосування немає), наприклад:
  ```bash
  docker exec -i walp-postgres psql -U $POSTGRES_USER -d $POSTGRES_DB < database/migrations/01-skill-tree-v2.sql
  ```
...
- `PORT` — задається окремо для кожного сервіса в docker-compose (3001–3008)
