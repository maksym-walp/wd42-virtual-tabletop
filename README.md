...
Для кожного бекенд-сервіса (`auth`, `user-profile`, `spellbook`, `skill-tree`, `character-sheet`, `dice-roller`, `equipment`, `maneuvers`):
```bash
cd services/<service>
npm install
...
```bash
cd services/<service>   # auth | user-profile | spellbook | skill-tree | character-sheet | dice-roller | equipment | maneuvers
npm test
```

Прогнати тести всіх бекенд-сервісів послідовно:
```bash
for s in auth user-profile spellbook skill-tree character-sheet dice-roller equipment maneuvers; do
