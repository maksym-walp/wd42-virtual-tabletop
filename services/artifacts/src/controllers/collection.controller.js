...
const CollectionController = {
  ...
  async addItem(req, res) {
    ...
    if (!artifact_id) return res.status(400).json({ message: 'artifact_id є обовʼязковим' });
    const added = await CollectionModel.addItem(req.params.id, req.user.sub, artifact_id, req.user.role === 'admin');
    if (!added) return res.status(404).json({ message: 'Колекцію або артефакт не знайдено' });
