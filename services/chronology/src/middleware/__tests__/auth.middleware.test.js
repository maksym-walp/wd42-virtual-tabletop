...
const { requireAuth, requireChronologyManager } = require('../auth.middleware');
...
describe('requireChronologyManager', () => {
  it('rejects a regular user', () => {
    jwt.verify.mockReturnValue({ sub: 'u1', role: 'user' });
    const req = { headers: { authorization: 'Bearer good' } };
    const res = mockRes();
    const next = jest.fn();
    requireChronologyManager(req, res, next);
  ...
  it('rejects a missing Authorization header before checking role', () => {
    const req = { headers: {} };
    const res = mockRes();
    const next = jest.fn();
    requireChronologyManager(req, res, next);
