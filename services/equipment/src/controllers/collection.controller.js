...
const CollectionController = {
  ...
  async addItem(req, res) {
    ...
    const added = await CollectionModel.addItem(req.params.id, req.user.sub, item_id);
    if (!added) return res.status(404).json({ message: 'Колекцію або предмет не знайдено' });
