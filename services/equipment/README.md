...
Сервіс каталогу спорядження: зброя, обладунки та звичайні предмети (кожен вид — окрема таблиця й окремий ендпоінт) плюс колекції (набори спорядження будь-яких видів, з опційним публічним доступом за посиланням). Порт **3007**, проксується Nginx як `/api/equipment/`.

## Ендпоінти

Кожен вид спорядження змонтовано під власним префіксом (`/items`, `/weapons`, `/armor`), колекції — під `/collections`, а корінь сервіса лишається спільним **читальним** зрізом по всіх трьох таблицях. Зовні це `/api/equipment/weapons/...`, `/api/equipment/collections/...` тощо.

### Спорядження за видами (`src/routes/catalog.routes.js`, `src/controllers/catalog.controller.js`)

Однаковий набір маршрутів для трьох префіксів; відрізняються лише поля, які має конкретний вид.

| Метод | Шлях | Авторизація | Тіло запиту | Відповідь |
|---|---|---|---|---|
| GET | `/{items\|weapons\|armor}` | Bearer JWT | — (query: `search`, `sort`, `dir`, `scope`; для зброї ще `weapon_type`, для обладунків `armor_weight`) | `200 { items: [...] }` — власне спорядження користувача + публічне |
| GET | `/{вид}/:id` | Bearer JWT | — | `200 { item }` / `404` якщо не знайдено або не видно (не власне і не публічне) |
| POST | `/{вид}` | Bearer JWT | `{ name (обов'язково), description?, is_public?, price?, image_url? }` + поля виду: зброя — `damage_die?`, `weapon_type?`, `weapon_grip?`; обладунки — `defense_value?`, `armor_weight?` | `201 { item }` / `400` якщо відсутнє `name` |
...
Поля, яких у виду немає, просто ігноруються — таблиця не має для них колонок.
...
| Метод | Шлях | Авторизація | Тіло запиту | Відповідь |
|---|---|---|---|---|
| GET | `/` | Bearer JWT | — (query: `search`, `sort`, `dir`, `scope`) | `200 { items: [...] }` — усі три види разом |
| GET | `/:id` | Bearer JWT | — | `200 { item }` / `404` якщо `id` немає в жодній із трьох таблиць або він не видний |

Кожен запис у відповіді (і в спільному зрізі, і в списках за видом) додатково містить `type` (`weapon` / `armor` / `item` — виводиться з таблиці, окремої колонки для нього немає), `is_owner` (чи належить поточному користувачу) та `is_canonical` (чи автор — адмін/GM, або чи запис явно позначено канонічним).
...
Колекція збирає спорядження будь-яких видів разом. Вид передавати не треба: сервіс сам визначає, у якій із трьох таблиць лежить `item_id`, і записує це в `item_kind`.
...
Сервіс володіє схемою `equipment`. Спорядження розкладене по трьох таблицях — по одній на вид (`39-equipment-split-tables.sql`); колонки `type` немає, бо вид визначає сама таблиця. Спільні для всіх колонки: `name`, `description`, `is_public`, `is_canonical`, `price`, `image_url`, `user_id`.
...
- `equipment.collection_items` — звʼязка many-to-many між колекціями та спорядженням: `collection_id`, `item_id` + `item_kind` (`item` / `weapon` / `armor` — у якій із трьох таблиць шукати рядок). FK на каталог тут немає (одна колонка не може посилатися на три таблиці), тож звʼязки прибирає сама модель при видаленні запису — див. `deleteQuery` у `src/models/catalog.model.js`.
