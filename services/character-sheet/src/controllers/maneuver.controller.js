...
const ManeuverController = {
  ...
  async add(req, res) {
    ...
    if (!await isVisibleToUser('abilities.maneuvers', maneuver_id, req.user.sub)) {
      return res.status(404).json({ message: 'Маневр не знайдено' });
    }
    const { met, missing } = await checkPrerequisites(req.params.id, 'abilities.maneuvers', maneuver_id);
