...
export default function CalendarBuilder() {
  ...
  const weekdays = useEditableList((d) => ({ name: '', ...d }));
  ...
  const loadAll = async () => {
    ...
    weekdays.setItems(weekdaysList.map((w) => ({ id: w.id, name: w.name })));
  ...
  const handleSave = async () => {
    ...
    try {
      for (const [i, wd] of weekdays.items.entries()) {
        const payload = { name: wd.name.trim(), order_num: i };
        if (wd.id.startsWith('new-')) await calendarApi.createWeekday(id, payload);
        else await calendarApi.updateWeekday(id, wd.id, payload);
      }
      for (const wdId of weekdays.deletedIds) await calendarApi.removeWeekday(id, wdId);
      ...
      for (const mo of moons.items) {
        ...
        if (mo.id.startsWith('new-')) await calendarApi.createMoon(id, payload);
        else await calendarApi.updateMoon(id, mo.id, payload);
      }
      for (const moonId of moons.deletedIds) await calendarApi.removeMoon(id, moonId);
      ...
      for (const [i, m] of months.items.entries()) {
        ...
        if (m.id.startsWith('new-')) {
          const created = await calendarApi.createMonth(id, payload);
          monthIdMap[m.id] = created.id;
        } else {
          await calendarApi.updateMonth(id, m.id, payload);
          monthIdMap[m.id] = m.id;
        }
      }
      for (const monthId of months.deletedIds) await calendarApi.removeMonth(id, monthId);
      ...
      for (const s of seasons.items) {
        ...
        if (s.id.startsWith('new-')) await calendarApi.createSeason(id, payload);
        else await calendarApi.updateSeason(id, s.id, payload);
      }
      for (const seasonId of seasons.deletedIds) await calendarApi.removeSeason(id, seasonId);
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 pb-32 sm:px-6 md:pb-8">
      <Link to="/calendars" className="mb-3 inline-flex items-center gap-1.5 text-sm text-text-dim">
      ...
      <div className="flex flex-col gap-4">
        ...
        <FormSection title="Дні тижня">
          <div className="flex flex-col gap-3">
            ...
            {weekdays.items.map((wd, i) => (
              ...
              >
                <Field label="Назва" className="flex-1">
        ...
        <div className="fixed inset-x-0 bottom-[calc(4.25rem+env(safe-area-inset-bottom))] z-30 flex justify-end gap-3 border-t border-border bg-surface px-4 py-3 md:static md:border-0 md:bg-transparent md:px-0 md:py-0">
          <Button type="button" variant="ghost" to="/calendars">Скасувати</Button>
