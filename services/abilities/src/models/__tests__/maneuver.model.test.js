...
describe('ManeuverModel.delete', () => {
  it('unlinks the entry from collections in the same statement, since the FK no longer cascades', async () => {
    ...
    client.query
      ...
      .mockResolvedValueOnce({ rows: [{ id: 'm1' }] })     // the delete itself
      .mockResolvedValue({ rows: [] });                    // trash insert + COMMIT

    await expect(ManeuverModel.delete('m1', 'u1')).resolves.toBe(true);

    const deleteCall = client.query.mock.calls.map(([sql]) => sql).find((sql) => sql && /DELETE FROM abilities\.maneuvers/.test(sql));
