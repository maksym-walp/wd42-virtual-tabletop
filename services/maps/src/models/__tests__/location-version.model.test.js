...
describe('LocationVersionModel.add', () => {
  it('inserts location id, year range, lore and override fields', async () => {
    pool.query.mockResolvedValueOnce({ rows: [{ id: 'v1' }] });
    await LocationVersionModel.add('loc1', {
      startYear: 600, endYear: 650, description: 'Ruins', gmNote: 'sunk', imageUrl: '/uploads/r.jpg',
      name: 'Ruins of X', markerIcon: '🏚', markerLevel: 2, types: ['ruin'],
    });
    const [sql, params] = pool.query.mock.calls[0];
    expect(sql).toMatch(/INSERT INTO maps\.location_versions/);
    expect(params).toEqual(['loc1', 600, 650, 'Ruins', 'sunk', '/uploads/r.jpg', 'Ruins of X', '🏚', 2, ['ruin']]);
  });

  it('passes a null base version + null overrides through', async () => {
    pool.query.mockResolvedValueOnce({ rows: [{ id: 'v1' }] });
    await LocationVersionModel.add('loc1', { startYear: null, description: null, gmNote: null, imageUrl: null });
    expect(pool.query.mock.calls[0][1]).toEqual(['loc1', null, null, null, null, null, null, null, null, null]);
  });
});
...
describe('LocationVersionModel.update', () => {
  it('scopes the update by version id and location id', async () => {
    pool.query.mockResolvedValueOnce({ rows: [{ id: 'v1' }] });
    await LocationVersionModel.update('v1', 'loc1', { startYear: 700, endYear: null, description: 'x', gmNote: null, imageUrl: null, name: null, markerIcon: null, markerLevel: null, types: null });
    const [sql, params] = pool.query.mock.calls[0];
    expect(sql).toMatch(/WHERE id = \$1 AND location_id = \$2/);
    expect(params).toEqual(['v1', 'loc1', 700, null, 'x', null, null, null, null, null, null]);
  });
