...
const CalendarMoonModel = {
  async findAllByCalendar(calendarId) {
    const { rows } = await pool.query(
      `SELECT * FROM calendar.calendar_moons WHERE calendar_id = $1 ORDER BY name ASC`,
      [calendarId]
    );
    return rows;
  },
  ...
  async create(calendarId, data) {
    ...
    const { rows } = await pool.query(
      `INSERT INTO calendar.calendar_moons (calendar_id, name, cycle_length, shift, color)
       VALUES ($1,$2,$3,$4,$5)
       RETURNING *`,
  ...
  async update(id, calendarId, data) {
    const { name, cycle_length, shift, color } = data;
    const { rows } = await pool.query(
      `UPDATE calendar.calendar_moons
  ...
  async delete(id, calendarId) {
    const { rowCount } = await pool.query(
      `DELETE FROM calendar.calendar_moons WHERE id=$1 AND calendar_id=$2`,
      [id, calendarId]
    );
    return rowCount > 0;
  },
...
module.exports = CalendarMoonModel;
