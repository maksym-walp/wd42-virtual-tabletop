...
const CalendarSeasonModel = {
  async findAllByCalendar(calendarId) {
    const { rows } = await pool.query(
      `SELECT s.*, m.name AS start_month_name
       FROM calendar.calendar_seasons s
       JOIN calendar.calendar_months m ON m.id = s.start_month_id
  ...
  async create(calendarId, data) {
    ...
    const { rows } = await pool.query(
      `INSERT INTO calendar.calendar_seasons (calendar_id, name, start_month_id, start_day, color, bg_image_url)
       VALUES ($1,$2,$3,$4,$5,$6)
       RETURNING *`,
  ...
  async update(id, calendarId, data) {
    const { name, start_month_id, start_day, color, bg_image_url } = data;
    const { rows } = await pool.query(
      `UPDATE calendar.calendar_seasons
  ...
  async delete(id, calendarId) {
    const { rowCount } = await pool.query(
      `DELETE FROM calendar.calendar_seasons WHERE id=$1 AND calendar_id=$2`,
      [id, calendarId]
    );
    return rowCount > 0;
  },
...
module.exports = CalendarSeasonModel;
